...
*Setup instructions for a Gitian build of Bobachain Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bobachain
...
- [Building Bobachain Core](#building-dash-core)
...
Clone the git repositories for Bobachain Core and Gitian.
...
in the Bobachain Core repository under 'Fetch and build inputs' to install sources which require
...
To build Bobachain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Bobachain Core repository.
