...
const char * const BITCOIN_CONF_FILENAME = "bobachain.conf";
const char * const BITCOIN_PID_FILENAME = "bobachaind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            if(ptrCategory->count(string("bobachain"))) {
...
#else
    const char* pszModule = "bobachain";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BobachainCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BobachainCore
    // Mac: ~/Library/Application Support/BobachainCore
    // Unix: ~/.bobachaincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BobachainCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BobachainCore";
#else
    // Unix
    return pathRet / ".bobachaincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bobachain.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bobachain.conf
